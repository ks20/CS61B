...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Iterator<Integer> segmentIterator = segment.iterator();
        String signalEnd = " ";
        int xPos = segmentIterator.next();
        int yPos = segmentIterator.next();
        Road road = _map.getLabel(xPos, yPos);

        String roadName = road.toString();
        String direction = road.direction().fullName();
        ...
        while (segmentIterator.hasNext()) {
            xPos = yPos;
            yPos = segmentIterator.next();
            Road curRoad = _map.getLabel(xPos, yPos);
            newName = curRoad.toString();
            newDir = curRoad.direction().fullName();
            curMiles = curRoad.length();
            if (newName.equals(roadName) && newDir.equals(direction)) {
                roadLength += curMiles;
            } else {
                System.out.println(seq + ". Take " + roadName + " "
                        + direction + " for "
                        + Math.round((int) ((roadLength * 10) + 0.5)) / 10.0
                        + " miles.");
                seq = seq + 1;
                roadName = newName;
                direction = newDir;
            ...
            lastName = _map.getLabel(yPos).toString();
        }
        System.out.println(seq + ". Take " + newName + " "
                + newDir + " for "
                + Math.round((int) ((roadLength * 10) + 0.5)) / 10.0
                + " miles to " + lastName + ".");
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road edg1 = new Road(name, dir, length);
        Road edg2 = new Road(name, dir.reverse(), length);
