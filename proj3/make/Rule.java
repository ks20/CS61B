...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!(commands.isEmpty()) && !(_commands.isEmpty())) {
            throw new IllegalStateException("error: Illegal to add.");
        }
    ...
    private void checkFinishedDependencies() {
        for (int successor : _depends.successors(_vertex)) {
            if (_depends.getLabel(successor).isUnfinished()) {
                error("error: Dependencies not built.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        if (_time == null) {
            ...
            for (int checkAll : _depends.successors(_vertex)) {
                if (_depends.getLabel(checkAll).getTime() == null) {
                    return true;
                }
                if (_depends.getLabel(checkAll).getTime() > _time) {
                    return true;
                }
            }
        }
